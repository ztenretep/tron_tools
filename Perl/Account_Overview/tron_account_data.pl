...
sub print_accountdates {
    ...
    my $tron = $_[0];
    my $decoded = $_[1];
    my @data = $tron->parse_account_dates($decoded);
...
sub print_accountfrozen {
    ...
    my $tron = $_[0];
    my $decoded = $_[1];
    my @data = $tron->parse_account_frozen($decoded);
...
sub print_accountvotes {
    ...
    my $fmtstr = "%-13s%s\n\n";
    # Get the data from the response.
    my $data = $tron->parse_account_votes($decoded);
    #my $sr_hash = @data;
    # Print the data into the terminal window.
...
# ===========================
# Subroutine print_totalvotes
# ===========================
sub print_totalvotes {
    # Assign the arguments to the local variables.
    my $tron = $_[0];
    my $decoded = $_[1];
    # Declare the array.
    ...
    my $total_votes = $tron->parse_total_votes($decoded);
    # Print the data into the terminal window.
...
sub main {
    ...
    my $tron = tron_account_data->new(%request);
    my $decoded = $tron->get_service_response->decode();
    # Print account frozen.
    print_accountfrozen($tron, $decoded);
    print "\n";
    # Print reward.
    print_reward();
    print "\n";
    ...
    print "\n";
    # Print total votes.
    print_totalvotes($tron, $decoded);
    print "\n";
    # Print account votes.
    print_accountvotes($tron, $decoded);
    print "\n";
    # Print account overview.
    print_accountdates($tron, $decoded);
};
